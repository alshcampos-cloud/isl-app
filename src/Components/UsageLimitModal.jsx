...
const UsageLimitModal = ({ user, supabase, userTier, onUpgrade, onClose, usageStats: propUsageStats }) => {
  const [loading, setLoading] = useState(true);
  const [usageData, setUsageData] = useState(null);
  const [error, setError] = useState(null);

  useEffect(() => {
    // COMPREHENSIVE FIX: If usageStats provided as prop, use it directly (synchronous)
    // This prevents async loading that breaks after tab switch
    if (propUsageStats) {
      setUsageData(propUsageStats);
      setLoading(false);
      return;
    }
    
    // Otherwise load async (for backwards compatibility)
    loadUsageData();
  }, [user, userTier, propUsageStats]);
